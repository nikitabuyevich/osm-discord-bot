...
module.exports = async (member) => {
  try {
    await pool(async (conn) => {
      ...
      // const [isMuted] = await conn.query('SELECT * FROM incidents WHERE affected_user = ? AND punishment_expires > current_timestamp ORDER BY created DESC LIMIT 1', [member.id]);
      if (isMuted) {
